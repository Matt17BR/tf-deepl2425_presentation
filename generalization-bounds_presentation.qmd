...
for i, d in enumerate(degrees_to_visualize):
    ...
    axes_top[i].set_title(f'Polynomial (Degree {d})', fontsize=16, pad=10)
...
ax_err.set_title('Training vs. Test Errors', fontsize=16, pad=10)
...
- **Breakdown**:
  ...
  - $\sum_{h \in \mathcal{H}} P(|R(h) - R_{\text{emp}}(h)| > \epsilon)$: Sums up probabilities of large error differences for each model.
